...
for(t in treats){
  ...
  cat(paste0('  Treatment: ', paste0('j', t, 'PRESERVE', outparty), '\n'))
  cat(paste0('   Outcome: ', y, '\n'))
  
  ## Check if this effect has already been estimated, and skip if so
  if(length(which(ef$outcome == y & ef$mask == m & ef$treatment ==  paste0('j', t, 'PRESERVE', outparty))) == 0){
...
t <- 'jrepublican.jdemocrat'; y <- 'pro_defendant'
